...
const RAID_CHANCE_PER_TICK: f32 = 0.01;
const RAID_TARGET_POP: usize = 6;
const WEATHER_PLAYER_TIME: f64 = 13.0;
const IDEA_TIME: f64 = 24.0;

const ANIMAL_MAX: usize = 120;
const ANIMAL_BREED_EVERY: u64 = 2160;
...
const WOLF_BITE_CHANCE: f32 = 0.0133;
...
const SAWMILL_CHOP_BONUS: f32 = 0.5;
...
const FAITH_GAIN_PER_TICK: f32 = 0.6;
const RITUAL_EVERY: u64 = 2160;
const FAITH_SPEND: f32 = 40.0;
const BLESS_LEN: f64 = 2160.0;
...
fn is_food_source(c: &Cell) -> bool {
    c.terrain == Terrain::Farm || c.terrain == Terrain::Berries || c.terrain == Terrain::Swamp || c.terrain == Terrain::CoralReef
}
...
const SOCIAL_LINK_CHANCE: u64 = 1200;
...
impl Sim {
    ...
    fn make_terrain(rng: &mut u64) -> Vec<Cell> {
        let mut grid = vec![
            Cell { terrain: Terrain::Grass, food: FOOD_MAX, ore: 0.0, water: 0.0, wood: 0.0, burn: 0, gold: 0.0 };
            W * H
        ];
        ...
        let mut brng = *rng;
        for _ in 0..48 {
            let start = (rfrac(&mut brng) as f64 * (W * H) as f64).floor() as usize % (W * H);
            if biome_map[start] != 0 || grid[start].terrain != Terrain::Grass {
                continue;
            }
            let mut group = 3 + (rfrac(&mut brng) * 4.0) as usize;
            let sx = (start % W) as i32;
            let sy = (start / W) as i32;
            let mut queue = vec![(sx, sy)];
            while group > 0 {
                let Some((qx, qy)) = queue.pop() else { break };
                if !in_bounds(qx, qy) {
                    continue;
                }
                let qi = idx(qx, qy);
                if grid[qi].terrain != Terrain::Grass || biome_map[qi] != 0 {
                    continue;
                }
                grid[qi].terrain = Terrain::Berries;
                grid[qi].food = BERRY_MAX;
                grid[qi].ore = 0.0;
                group -= 1;
                let d = rfrac(&mut brng);
                queue.push((qx + if d < 0.5 { 1 } else { -1 }, qy));
                queue.push((qx, qy + if d < 0.5 { -1 } else { 1 }));
            }
        }
        for c in grid.iter_mut() {
            c.wood = match c.terrain {
                Terrain::Forest => WOOD_MAX,
                Terrain::Jungle => WOOD_MAX * 1.5,
                _ => 0.0,
            };
    ...
    fn spawn_world(&mut self) {
        ...
        for (i, &(r, g, b)) in PALETTE.iter().enumerate() {
            ...
            for k in 0..10 {
                let ang = k as f32 * 0.6283 + i as f32 * 0.4;
                let dx = (ang.cos() * 6.0).round() as i32;
                let dy = (ang.sin() * 5.5).round() as i32;
                let (x, y) = (cx + dx, cy + dy);
                if in_bounds(x, y) && self.grid[idx(x, y)].terrain == Terrain::Grass {
                    if k < 7 {
                        self.grid[idx(x, y)].terrain = Terrain::Forest;
                        self.grid[idx(x, y)].food = 0.0;
                        self.grid[idx(x, y)].wood = WOOD_MAX;
                    } else {
                        self.grid[idx(x, y)].terrain = Terrain::Berries;
                        self.grid[idx(x, y)].food = BERRY_MAX;
                    }
                }
            }
    ...
    pub fn bless(&mut self, gx: i32, gy: i32, r: i32) {
        for dy in -r..=r {
            for dx in -r..=r {
                ...
                match c.terrain {
                    Terrain::Grass => {
                        let h = (nx as u32).wrapping_mul(7).wrapping_add((ny as u32).wrapping_mul(13)) % 10;
                        if h < 7 {
                            c.terrain = Terrain::Forest;
                            c.food = 0.0;
                            c.wood = WOOD_MAX;
                        } else {
                            c.terrain = Terrain::Berries;
                            c.food = BERRY_MAX;
                            c.wood = 0.0;
                        }
                    }
    ...
    pub fn tick(&mut self) {
        ...
        if self.tick_count % RECALC_TERRITORY_EVERY == 0 || self.tick_count == 1 {
        ...
        if self.tick_count % REGROW_EVERY == 0 {
            ...
            let water_regen = if rain { WATER_REGEN_RAIN } else { 0.0 }
                + if self.season == Season::Spring { WATER_REGEN_SPRING_MELT } else { 0.0 }
                + if proph_rain { 30.0 } else { 0.0 };
            ...
            for y in 0..H {
                for x in 0..W {
                    ...
                    match self.grid[i].terrain {
                        ...
                        Terrain::Forest | Terrain::Jungle | Terrain::Berries | Terrain::Hills | Terrain::Grass | Terrain::Volcano => {}
        ...
        if self.tick_count % FOREST_REGROW_EVERY == 0 {
            for (i, c) in self.grid.iter_mut().enumerate() {
                match c.terrain {
        ...
        for (ti, t) in self.towns.iter_mut().enumerate() {
            ...
            if t.prophecy == Prophecy::Prosperity && self.tick_count % PROSPERITY_EVERY == 0 {
                let gold_cap = stock_cap(&t.built, ResourceKind::Gold);
    ...
    fn weather_breath(&mut self) {
        ...
        self.weather_left = 72.0 + rfrac(&mut self.rng) as f64 * 96.0;
    }
    ...
    pub fn load_json(json: &str) -> Option<Self> {
        ...
        sim.migrate_legacy_wood();
        sim.rebuild_cache();
        sim.rebuild_fire_cells();
        Some(sim)
    }

    fn migrate_legacy_wood(&mut self) {
        for c in self.grid.iter_mut() {
            match c.terrain {
                Terrain::Forest if c.wood <= 0.0 => c.wood = c.food.max(0.5).min(WOOD_MAX),
                Terrain::Jungle if c.wood <= 0.0 => c.wood = c.food.max(0.5).min(WOOD_MAX * 1.5),
                _ => {}
            }
            if c.terrain == Terrain::Forest || c.terrain == Terrain::Jungle {
                c.food = 0.0;
    ...
    fn bite_agent(&mut self, ax: i32, ay: i32) {
        ...
        for a in self.agents.iter_mut() {
            if (a.x - ax).abs().max((a.y - ay).abs()) <= 1 {
                a.hunger = (a.hunger + 60.0).min(HUNGER_MAX);
            }
        }
    }
    ...
    fn decide(&self, a: &Agent) -> (Action, ResourceKind) {
        if a.sick > 0 && (a.hunger >= 360.0 || a.thirst >= 80.0) {
            return (Action::Die, a.want);
        }
        if a.hunger >= STARVE_HUNGER || a.thirst >= STARVE_THIRST {
            return (Action::Die, a.want);
        }
        if a.raider {
    ...
    fn most_needed(&self, ti: usize) -> ResourceKind {
        ...
        let fi = s.fish / 10.0;
        let wd = s.wood / 15.0;
        if w < f && w <= o && w <= m && w <= fi && w <= wd {
            ResourceKind::Water
        } else if fi < f && fi <= o && fi <= m && fi <= wd {
            ResourceKind::Fish
        } else if m < f && m <= o && m <= wd {
            ResourceKind::Meat
        } else if o < f && o <= wd {
            ResourceKind::Ore
        } else if wd < f {
    ...
    fn resource_target(&self, x: i32, y: i32, kind: ResourceKind) -> Option<(i32, i32)> {
        match kind {
            ...
            ResourceKind::Wood => self.seek(x, y, |s, nx, ny| {
                let c = &s.grid[idx(nx, ny)];
                (c.terrain == Terrain::Forest || c.terrain == Terrain::Jungle) && c.wood > 1.0
            }),
                ...
                let on_road = self.roads[idx(nx, ny)];
                {
                    ...
                    if a.carry.is_none() {
                        match a.want {
                            ...
                            ResourceKind::Wood => {
                                let c = &self.grid[idx(nx, ny)];
                                if (c.terrain == Terrain::Forest || c.terrain == Terrain::Jungle) && c.wood > 1.0 {
                                    let saw = self.towns[a.home].built.iter().filter(|b| **b == BuildingKind::Sawmill).count();
                                    let qty = (1.0 + SAWMILL_CHOP_BONUS * saw as f32).min(4.0).min(c.wood);
                                    self.grid[idx(nx, ny)].wood -= qty;
                                    a.carry = Some((ResourceKind::Wood, qty));
                                }
                            }
                        }
                    }
                    ...
                    for dx in -1..=1 {
                        ...
                        if self.grid[ni].burn == 0
                            && (self.grid[ni].terrain == Terrain::Forest
                                ...
                                || self.grid[ni].terrain == Terrain::Berries)
                        {
                            ignites.push(ni);
                        }
                    }
        ...
        for k in 0..self.agents.len() {
            ...
            if (foe.2 - my_x).abs() <= 1 && (foe.3 - my_y).abs() <= 1 {
                ...
                if rfrac(&mut self.rng) < kill_chance {
                    self.agents[k].hunger = STARVE_HUNGER;
                }
                return;
            }
        }
...
mod tests {
    ...
    #[test]
    fn bless_creates_forest_and_berry_fields() {
        let mut s = Sim::new(3);
        ...
        for dy in -2..=2 {
            for dx in -2..=2 {
                ...
                match s.grid[idx(cx, cy)].terrain {
                    Terrain::Forest => {
                        forest += 1;
                        assert_eq!(s.grid[idx(cx, cy)].wood, WOOD_MAX, "forest should hold wood");
                    }
                    Terrain::Berries => {
                        berries += 1;
                        assert_eq!(s.grid[idx(cx, cy)].food, BERRY_MAX, "berries should hold food");
                    }
                    other => panic!("blessed grass should become forest or berries, got {:?}", other),
                }
            }
        }
        assert!(forest > 0 && berries > 0, "blessing should make both forest and berry fields (f{} b{})", forest, berries);
    }
    ...
    fn construction_builds_outpost_as_map_object() {
        let mut s = Sim::new(15);
        s.towns[0].stocks.food = 400.0;
        s.towns[0].stocks.water = 400.0;
        s.towns[0].stocks.ore = 400.0;
        s.towns[0].stocks.wood = 300.0;
    ...
    fn combat_kills_defenders_and_loots() {
        ...
        for _ in 0..60 {
            s.combat_check(ridx);
            if s.agents.iter().any(|a| a.home == 1 && a.hunger >= STARVE_HUNGER) {
                killed = true;
                break;
            }
        }
    ...
    fn collapsed_town_ruins_over_ticks() {
        ...
        s.rebuild_cache();
        let mut died = false;
        for e in 1..=TOWN_WASTE_NEED as u64 {
            s.tick_count = e * TOWNS_EVERY;
            s.town_lifecycle();
            if !s.towns[0].alive {
                died = true;
                break;
    ...
    #[test]
    fn forest_wood_only_regrows_every_decade() {
        ...
        s.grid[idx(fx1, fy1)].wood = 2.0;
        s.grid[idx(fx2, fy2)].wood = WOOD_MAX - 1.0;
        s.tick();
        assert!(
            s.grid[idx(fx1, fy1)].wood < WOOD_MAX && s.grid[idx(fx2, fy2)].wood < WOOD_MAX,
            "forest must NOT regrow daily: got {:.1} and {:.1}",
            s.grid[idx(fx1, fy1)].wood,
            s.grid[idx(fx2, fy2)].wood
        );
        s.tick_count = FOREST_REGROW_EVERY - 1;
        s.tick();
        assert_eq!(s.grid[idx(fx1, fy1)].wood, WOOD_MAX, "picked forest should fully regrow at the decade mark");
        assert_eq!(s.grid[idx(fx2, fy2)].wood, WOOD_MAX, "full forest should stay max at the decade mark");
    }
    ...
    fn population_does_not_collapse() {
        for seed in 1..10u64 {
            ...
            let window = DAY_LEN as usize;
            for _ in 0..60 * DAY_LEN {
                s.tick();
                ...
                if samples.len() == window {
                    worst = worst.min(samples[0] - samples[window - 1]);
                }
            }
            eprintln!("seed {} base {} worst{}drop {} end {}", seed, base, window, -worst, s.agents.len());
    ...
    fn population_grows_over_long_run() {
        ...
        assert!(peak > start, "world should grow over the long run (start {} peak {})", start, peak);
        assert!(peak > trough, "population should recover from any dip (trough {} peak {})", trough, peak);
        assert!(s.agents.len() > start, "final population should exceed start");
    }
    ...
    #[test]
    fn death_thresholds_match_five_day_hunger_and_two_day_thirst() {
        let mut s = Sim::new(802);
        prep_two_towns(&mut s);
        s.agents[0].x = s.towns[0].x;
        s.agents[0].y = s.towns[0].y;
        s.agents[0].hunger = STARVE_HUNGER - 1.0;
        s.agents[0].thirst = 0.0;
        let (act, _) = s.decide(&s.agents[0]);
        assert!(
            !matches!(act, Action::Die),
            "just below hunger threshold should not kill (got {:?})",
            act
        );
        s.agents[0].hunger = STARVE_HUNGER;
        s.agents[0].thirst = 0.0;
        let (act, _) = s.decide(&s.agents[0]);
        assert!(matches!(act, Action::Die), "hunger death threshold reached");
        assert_eq!(
            STARVE_HUNGER,
            30.0 + 3.75 * (5.0 * DAY_LEN as f32),
            "hunger death should land at 5 days from the post-meal baseline"
        );
        s.agents[0].hunger = 0.0;
        s.agents[0].thirst = STARVE_THIRST - 1.0;
        let (act, _) = s.decide(&s.agents[0]);
        assert!(
            !matches!(act, Action::Die),
            "just below thirst threshold should not kill (got {:?})",
            act
        );
        s.agents[0].hunger = 0.0;
        s.agents[0].thirst = STARVE_THIRST;
        let (act, _) = s.decide(&s.agents[0]);
        assert!(matches!(act, Action::Die), "thirst death threshold reached");
        assert_eq!(
            STARVE_THIRST,
            30.0 + 1.25 * (2.0 * DAY_LEN as f32),
            "thirst death should land at 2 days from the post-drink baseline"
        );
    }
    ...
    fn child_becomes_worker_after_growing() {
        ...
        s.agents[0].age = crate::sim::CHILD_AGE - 2;
    ...
    fn barracks_ordains_guard() {
        ...
        for _ in 0..250 {
            s.tick();
            if s.towns[0].built.iter().any(|b| *b == BuildingKind::Barracks) {
                built = true;
                break;
            }
        }
        assert!(built, "barracks should be built");
        assert!(
    ...
    fn sanctuary_ordains_priest() {
        ...
        s.towns[0].stocks = Stock { food: 500.0, water: 500.0, ore: 60.0, meat: 15.0, gold: 0.0, fish: 0.0, wood: 40.0 };
        for _ in 0..2 * DAY_LEN {
            s.tick();
        }
        assert!(
    ...
    fn clinic_ordains_healer() {
        ...
        for _ in 0..220 {
            s.tick();
            if s.towns[0].built.iter().any(|b| *b == BuildingKind::Clinic) {
                built = true;
                break;
            }
        }
        assert!(built, "clinic should be built");
        assert!(
...
fn migration_relocates_adults_to_better_town() {
        for seed in 1..60u64 {
            ...
            let adult = migration_agent(0, cx + 2 + (seed % 5) as i32, cy + (seed % 3) as i32, 200000, 0);
            let child = migration_agent(0, cx + 4, cy, 10, 0);
            let sick = migration_agent(0, cx + 6, cy, 200000, 5);
            s.agents.push(adult);
            s.agents.push(child);
            s.agents.push(sick);
    ...
    fn migration_is_deterministic_and_happens() {
        for seed in 1..20u64 {
            ...
            for k in 0..3i32 {
                let a1 = migration_agent(0, cx + 2 + k * 3, cy + (seed % 2) as i32, 200000, 0);
                let a2 = migration_agent(0, cx + 2 + k * 3, cy + (seed % 2) as i32, 200000, 0);
                s1.agents.push(a1);
                s2.agents.push(a2);
            }
            let mut moved = 0;
...
fn marriages_form_and_cheapen_births() {
    ...
    fn forest_fire_burns_and_expires_leaving_grass() {
        ...
        assert!(c.wood <= 0.01, "burned forest holds no wood");
    ...
    fn forest_does_not_regrow_on_road() {
        ...
        for dy in -10..=10 {
            for dx in -10..=10 {
                ...
                if in_bounds(fx, fy) && s.grid[idx(fx, fy)].terrain == Terrain::Forest {
                    ...
                    s.grid[idx(fx, fy)].wood = WOOD_MAX - 1.0;
                    s.tick_count = FOREST_REGROW_EVERY - 1;
                    s.tick();
                    assert_eq!(s.grid[idx(fx, fy)].wood, WOOD_MAX - 1.0, "forest wood on road must not regrow");
                    found_forest = true;
                    break;
                }
            }
        ...
        assert!(found_forest, "expected a forest near the town");
    }
    ...
    fn balance_10_sessions_20_years() {
        ...
        for seed in 1..=SESSIONS as u64 {
            ...
            for tick in 0..TICKS {
                s.tick();
                if seed == 1 && (tick % 5760 == 0 || tick == TICKS - 1) {
                    eprintln!("S1 tick {} pop {} towns {}", tick, s.agents.len(), s.towns.len());
                }
                let pop = s.agents.len();
                if pop > peak_pop { peak_pop = pop; }
